...
`conversationId` 可选。第一次不传时由后端创建并通过响应中的 `data.conversationId` 返回；后续问题携带相同值即可进入同一个会话。后端会用“滚动摘要 + 最近 3 个完整成功轮次”把“那华东呢”“换成去年”等追问改写成完整独立问题，再进入 Text-to-SQL。
...
再次提交相同用户的相同问题，可以观察 `cacheHit` 变为 `true`。多轮追问使用改写后的独立问题作为缓存 Key，因此两个表达不同但最终语义相同的问题也可以安全复用同一份脱敏结果。
...
| 400 | 40003 | 仅支持只读查询 |
| 400 | 40004 | 检测到 Prompt 注入攻击 |
| 400 | 40005 | 模型上下文超过 Token 安全预算 |
...
后端会把 Prompt、预留输出和安全余量一起计入模型窗口预算，预计超过80%时先压缩早期上下文。压缩后仍超限会返回业务码 `40005`，不会静默截断最近轮次。此时应缩短当前问题或开始一个新会话。
