...
flowchart LR
    ...
    Controller --> Guard["Prompt 注入 / 意图 / 语义 / 限流"]
...
```text
Prompt 注入校验 → 写操作意图校验 → TopN 等语义校验 → Redis 限流
```

Prompt 注入校验会直接拒绝索取系统 Prompt、要求忽略规则、角色劫持、`unrestricted/jailbreak` 和要求生成危险 DDL/DML 等攻击，返回错误码 `40004`。它位于请求守卫第一步，因此攻击不会进入限流、缓存、DeepSeek 或业务数据库查询。其他非法请求同样应尽早拒绝，避免浪费 Redis 令牌和大模型费用。限流放在缓存之前，因为缓存命中虽然不调用模型，仍会消耗 Web、Redis 和审计资源，不能允许无限请求。完整查询接口仍可异步保存一条安全拒绝审计记录到系统库，这不是业务数据查询。

对应源码：

- [DefaultQueryRequestGuard](../src/main/java/com/aianalyst/service/impl/DefaultQueryRequestGuard.java)
- [QueryPromptInjectionValidator](../src/main/java/com/aianalyst/service/QueryPromptInjectionValidator.java)
- [RegexQueryPromptInjectionValidator](../src/main/java/com/aianalyst/service/impl/RegexQueryPromptInjectionValidator.java)
...
- [TextToSqlPromptBuilder](../src/main/java/com/aianalyst/service/prompt/TextToSqlPromptBuilder.java)
...
审核失败和执行失败都不能一律重试，两类纠错共享最多 2 次预算：

| 异常 | 是否请求模型修复 | 原因 |
|---|---|---|
| 多语句或 SQL 无法解析 | 是，最多 1 次 | 只修复模型输出格式并重新完整审核 |
| 排除权限错误后的 `BadSqlGrammarException` | 是，使用剩余额度 | 字段名或语法可能由模型修正 |
| 连接失败 | 否 | 重新生成 SQL 不能恢复网络 |
| 查询超时 | 否 | 盲目重试可能继续占用资源 |
| 权限不足 | 否 | 不能通过模型绕过权限 |
| 未授权表、危险函数、非 SELECT 等安全拒绝 | 否 | 不能让模型尝试绕过安全边界 |

分类时权限错误优先否决：沿异常链识别 Spring 权限异常、SQLState `28xxx` 和 MySQL 权限错误码，避免 MySQL 的 SQLState `42000` 被包装成 `BadSqlGrammarException` 后误触发模型修复。审核格式纠错和执行语法纠错合计最多 2 次，每次修复结果都重新经过完整 SQL 审核。候选 SQL 与数据库错误信息会截断，避免 Prompt 过大；原始错误不会直接返回前端。
...
| Prompt 注入攻击 | 请求守卫第一步直接拒绝 | 是，不调用模型和数据库 |
| Text-to-SQL 模型不可用 | 生成 SQL 失败 | 是，没有 SQL 无法继续 |
| 模型输出多语句/格式错误 | 最多纠错 1 次并重新审核 | 可能恢复 |
| SQL 执行语法错误 | 使用共享预算的剩余额度 | 可能恢复 |
| 数据库连接/权限错误 | 不做模型重试 | 是 |
...
| Prompt 注入测试 | [RegexQueryPromptInjectionValidatorTest](../src/test/java/com/aianalyst/service/impl/RegexQueryPromptInjectionValidatorTest.java) | 验证 Prompt 泄露、角色劫持、输出劫持及正常问题反向用例 |
...
当前全量测试共 131 项，默认跳过需要外部环境或显式开启的测试。详细的阶段性结果见 [Day24](day24-test-report.md) 和 [Day25](day25-performance-security-report.md)。
...
16. 为什么 Prompt 中已经写了“忽略攻击指令”，还需要模型调用前的独立注入校验？
...
16. Prompt 约束不是强安全边界，模型仍可能被诱导；前置校验可以避免攻击进入模型、减少 Prompt 泄露面和模型费用，后续 AST 与只读账号继续兜底。
...
- RabbitMQ 尚未接入；
- SQL 审核目前完成表级白名单，尚未实现基于 AST 的字段级白名单和普通 `SELECT *` 拦截；
- AI 总结仍同步影响主请求时延；
...
- [ ] 从业务元数据 YAML 构建 `表名 -> 允许字段集合`；
- [ ] 解析 `FROM`/`JOIN` 的表别名，并校验 `SELECT`、`WHERE`、`JOIN ON`、`GROUP BY`、`HAVING`、`ORDER BY` 中的物理字段；
- [ ] 区分物理字段与 `SELECT` 结果别名，避免误拒绝 `ORDER BY total_sales`；
- [ ] 拒绝普通 `SELECT *` 和 `table.*`，但允许 `COUNT(*)`；
- [ ] 覆盖子查询、CTE、聚合函数、未限定字段和同名字段歧义的测试；
- [ ] 未授权字段统一返回 SQL 审核失败，确保请求不会进入只读业务数据库。
