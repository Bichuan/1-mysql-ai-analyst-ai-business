...
> 第四个核心点是对 AI 和外部依赖做有限自纠错与分级降级。审核阶段和执行阶段共享最多两次纠错预算：模型若仅输出多条 SQL 或无法解析的格式，最多允许一次受约束的格式纠错；进入执行后，只有排除权限问题的 `BadSqlGrammarException` 才能使用剩余额度。未授权表、危险函数、非 SELECT、网络、超时和权限不足都不会重试。每次修复后的 SQL 必须重新经过完整安全审核，防止纠错通道成为绕过审核的入口。除此之外，缓存故障会降级为未命中，AI 总结失败会返回降级文案但保留查询数据，异步历史失败只记录日志。

### 5.2 为什么不能执行失败就全部重试

| 异常 | AI 修复 | 原因 |
|---|---|---|
| 多语句或 SQL 无法解析 | 最多 1 次 | 修复输出格式后重新完整审核 |
| 排除权限问题后的 SQL 语法/字段错误 | 使用剩余额度 | 模型可能修复 |
| 未授权表、危险函数、非 SELECT 等拒绝 | 不修复 | 不能尝试绕过安全规则 |
...
- [DataQueryServiceImpl](../src/main/java/com/aianalyst/service/impl/DataQueryServiceImpl.java)：查询编排、异常分类和共享纠错预算；
- [TextToSqlServiceImpl](../src/main/java/com/aianalyst/service/impl/TextToSqlServiceImpl.java)：生成、一次审核格式恢复与修复后重新审核；
...
答：纠错次数需要在成功率、响应时间和模型费用之间取舍。审核格式纠错最多一次，并与执行语法纠错共享两次总预算；这样既能恢复常见模型输出抖动，又不会让多个阶段分别重试造成模型调用失控。
...
>
> 我重点处理了几个工程问题。第一，Prompt 约束、AST 审核、查询超时和只读账号形成纵深安全，避免把安全寄托在模型上。第二，Redis 缓存按用户隔离，并明确它只是归一化文本的精确缓存；限流通过 Lua 原子实现固定窗口，而且缓存故障可降级、限流故障不放行。第三，多语句/格式错误最多修复一次，并与排除权限问题后的执行语法纠错共享两次总预算，每次修复都重新审核。第四，原始结果先脱敏，脱敏副本才能进入模型、前端、缓存和审计。第五，AI 总结失败会降级但保留数据，历史记录使用独立有界线程池异步写入。
>
...
- 实现审核格式与执行语法错误共享两次 AI 纠错预算、结果脱敏、AI 总结降级、异步审计、Request ID 与 Micrometer 指标。
